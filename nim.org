...
#+begin_src python :noweb-ref save-state-machine
  ...
  def _save_state_machine(indent: int, states: List[State], debug: bool, need_action_delegate: bool, need_guard_delegate: bool, need_variable_delegate: bool, prefix: str, asynced: bool) -> Tuple[str, str, str]:
    ...
    action_idx_mapping = {}
    for (x, row) in enumerate(actionstable):
      for (y, col) in enumerate(row):
        if col in action_idx_mapping:
          action_idx_mapping[col].append(len(row) * x + y)
        else:
          action_idx_mapping[col] = [len(row) * x + y]
    ...
    eventimpl += ' ' * delta + 'let newctx = case idx:\n'
    casebaseindent = len(' ' * delta + 'let newctx = ')
    for (key, value) in action_idx_mapping.items():
      if key != "noop":
        eventimpl += ' ' * (casebaseindent + delta) + 'of {idxs}:{awaitprefix} {action}[T]({args})\n'.format(idxs = ', '.join([str(x) for x in value]), awaitprefix = awaitprefix, action = key, args = ', '.join(['fsm', 'ctx'] + [x.split(':')[0] for x in action_parameter_signatures]))
      else:
        continue
