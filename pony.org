...
#+begin_src python :noweb-ref action-delegate
  def _save_action_delegate(varaibles: List[Variable], states: List[State], prefix: str) -> str:
    ...
    if code != '':
      code = 'trait ' + prefix + 'ActionDelegate[T: Any val]\n' + code + '\n'
    return code
#+end_src
**** guard delegate
#+begin_src python :noweb-ref guard-delegate
  def _save_guard_delegate(states: List[State], prefix: str) -> Tuple[List[str], str]:
    ...
    if code != '':
      code = 'trait ' + prefix + 'GuardDelegate[T: Any val]\n' + code + '\n'
    return code
#+end_src
**** variable delegate
#+begin_src python :noweb-ref variable-delegate
  def _save_variable_delegate(variables: List[Variable], prefix: str) -> Tuple[List[str], str]:
    ...
    if code != '':
      code = 'trait ' + prefix + 'VariableDelegate[T: Any val]\n' + code + '\n'
    return code
#+end_src
*** state machine
#+begin_src python :noweb-ref save-state-machine
  ...
  def _save_state_machine(states: List[State], need_action_delegate: bool, need_guard_delegate: bool, need_variable_delegate: bool, prefix: str) -> str:
    ...
    delegates_in_action = []
    if need_action_delegate:
      delegates_in_action.append('action_delegate: {0}ActionDelegate[T]'.format(prefix))
    if need_variable_delegate:
      delegates_in_action.append('variable_delegate: {0}VariableDelegate[T]'.format(prefix))
    actionimpl = 'interface _{0}Action[T: Any val]\n'.format(prefix)
    actionimpl += ' ' * 4 + 'fun apply({0}): T\n\n'.format(', '.join(delegates_in_action + ["ctx0: T"] + optional_action_parameter_signatures))
    actionimpl += 'primitive _{0}Noop[T: Any val]\n'.format(prefix)
    ...
    code = 'class {0}StateMachine[T: Any val]\n'.format(prefix)
    code += ' ' * 4 + 'var state: I32\n'
    delegates = []
    if need_action_delegate:
      code += ' ' * 4 + 'var action_delegate: {0}ActionDelegate[T]\n'.format(prefix)
      delegates.append('actiondelegate: {0}ActionDelegate[T]'.format(prefix))
    if need_guard_delegate:
      code += ' ' * 4 + 'var guard_delegate: {0}GuardDelegate[T]\n'.format(prefix)
      delegates.append('guarddelegate: {0}GuardDelegate[T]'.format(prefix))
    if need_variable_delegate:
      code += ' ' * 4 + 'var variable_delegate: {0}VariableDelegate[T]\n'.format(prefix)
      delegates.append('variabledelegate: {0}VariableDelegate[T]'.format(prefix))
